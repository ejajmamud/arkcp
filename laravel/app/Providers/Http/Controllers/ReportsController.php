...
class ReportsController extends Controller
{
    ...
    public function index($fildata)
    {
        ...
        Debugbar::info('$fildata'.$fildata);

        return view('reports', ['students'=>$student, 'fildata' => $fildata ,'datausd' => json_encode($dataUsd), 'datarm' => json_encode($dataRm), 'users'=>$allStudents, 'incomeRm' => $totalIncomeRm, 'completedTests' => $testsCompleted, 'income'=>$totalIncome, 'usersChartData'=>json_encode($usersChartData), 'testsChartData'=>json_encode($testsChartData)]);
    }
    public function getbyrange(Request $request){
        $from = date($request->from." 00:00:00");
        $to = date($request->to." 23:59:59");
        $fildata = Student::where('payment_status', '=', 'approved')->whereBetween('created_at', array($from, $to))->get();
        
        return $this->index($fildata);
